class Solution:
    def trap(self, height: List[int]) -> int:
        ...
        if height[left_pointer] > highest:
            highest = height[left_pointer]
        while right_pointer < len(height):
            if height[right_pointer] >= highest:
                ...
                is_right_pointer = right_pointer == len(height) - 1
                if ((height[right_pointer] in well) and not max_height >= height[left_pointer]) or is_right_pointer:
                    left_pointer += well.index(height[right_pointer]) + 1 if not is_right_pointer else 0
                    temp_sum = sum(height[left_pointer + 1: right_pointer])
                    total_water = min(height[left_pointer], height[right_pointer]) * (right_pointer - left_pointer - 1) - temp_sum
                    water += total_water if total_water > 0 else 0
                    lower_passed = not lower_passed
                    left_pointer = right_pointer
            right_pointer += 1
        return water